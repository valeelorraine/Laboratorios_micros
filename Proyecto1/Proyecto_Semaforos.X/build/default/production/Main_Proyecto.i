...
POP:
   ...
   RETFIE ; Regresar de la interr. (incluye reactivacion del ((INTCON) and 07Fh), 7)
...
;Button:
 ...
 ; BTFSS PORTB, 0 ; Revisar MODO (1er pin del PORTB)
  ; BSF FLAG, 0 ; Setear FLAG
   ...
   ; BCF INTCON, 0 ; Limpiar FLAG ((INTCON) and 07Fh), 0
   ; RETURN
...
weak_PU:
    ...
    MOVLW 00000111B ; Habilitar lo del IOCB en pines ((PORTB) and 07Fh), 0, ((PORTB) and 07Fh), 1 Y ((PORTB) and 07Fh), 2
...
Interrupciones:
    ...
    BCF INTCON, 0 ; ((INTCON) and 07Fh), 0 Limpiar la bandera de CHANGE INTERRUPT
    BCF INTCON, 2 ; ((INTCON) and 07Fh), 2 Limpiar la bandera del TMR0
    BSF INTCON, 3 ; ((INTCON) and 07Fh), 3 Encender interrupción PORTB CHANGE
    BSF INTCON, 5 ; ((INTCON) and 07Fh), 5 Encender interrupción de OVERFLOW TMR0
    BSF INTCON, 6 ; ((INTCON) and 07Fh), 6 Encender interrupción periférica
    BSF INTCON, 7 ; ((INTCON) and 07Fh), 7 Encender interrupción de global

    banksel OPTION_REG ; Configuraciones para TMR2
    BSF PIE1, 1 ; ((PIE1) and 07Fh), 0 Encender interrupción de OVERFLOW TMR2
    banksel TMR0
    BSF T2CON, 2 ; Encender el timer2
    BCF PIR1, 1 ; ((PIR1) and 07Fh), 1 TIMER2 to PR2 Interrupt Flag
...
PRE0:
    BCF ((OPTION_REG) and 07Fh), 3 ; Prescaler se le asigna al módulo TMR0
...
timer2:
    ...
    MOVWF PR2 ; Obtener un ((PIR1) and 07Fh), 1 de 2ms
    RETURN
